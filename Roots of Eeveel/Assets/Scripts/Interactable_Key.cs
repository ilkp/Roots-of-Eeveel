...
public class Interactable_Key : MonoBehaviour, IInteractable
{
    ...
    IEnumerator Hold()
    {
        ...
        while (true)
        {
            ...
            if (Input.GetKey(Keybindings.Instance.secondaryInteraction) || Input.GetKey(Keybindings.Instance.altSecondaryInteraction))
            {
    ...
    {
        if (collision.collider.GetComponent<PuzzleLock>() != null && collision.gameObject.GetComponent<PuzzleLock>().Identifier == lockIdentifier)
        {
